...
//  script.js — Full Supabase Backend Integration
// ============================================================
...
async function generateSerial() {
  ...
  // Get current count of paid students to make serial sequential
...
function updatePaginationInfo(count) {
  document.querySelector('.pagination-info').textContent =
    `Showing ${Math.min(count, count)} of ${count} student${count !== 1 ? 's' : ''}`;
}
      ...
      <div class="vip-student-card" data-name="${s.name.toLowerCase()}" data-serial="${s.serial}">
        ...
        <div class="vip-card-details">
          <div class="vip-detail-row">
            ...
            <span class="vip-detail-val" style="color:var(--success);font-weight:700">
...
function updateEditCalcs() {
  ...
  const balance = Math.max(0, total - paid);
  document.getElementById('edit-balance').value = '₦' + balance.toLocaleString('en-NG');
}
...
async function saveEditStudent() {
  ...
  // If now fully paid and wasn't before, generate serial
  let serial       = null;
  let serialActive = false;
  if (isFullyPaid) {
    const { data: existing } = await supabase
      .from('students').select('serial, serial_active').eq('id', _editStudentId).single();
  ...
  const { error } = await supabase.from('students').update(updates).eq('id', _editStudentId);
...
let _payStudentBalance = 0;

async function openPaymentModal(studentId) {
  _payStudentId = studentId;

  const { data: s } = await supabase
    .from('students').select('name, class, balance').eq('id', studentId).single();
...
async function saveNewPayment() {
  ...
  const { data: s } = await supabase
    .from('students').select('*').eq('id', _payStudentId).single();
  ...
  let serial       = s.serial;
  let serialActive = s.serial_active;
  ...
  showToast(
    isNowFullyPaid
      ? '✓ Full payment received! Serial number generated.'
...
async function executeDelete() {
  ...
  await supabase.from('payments').delete().eq('student_id', _deleteStudentId);

  const { error } = await supabase.from('students').delete().eq('id', _deleteStudentId);
...
async function executeRegenSerial() {
  ...
  // Log the old serial in serial_history, then update
  await supabase.from('serial_history').insert([{
    student_id:  _regenStudentId,
  ...
  showToast('Serial number regenerated successfully!', 'success');
  loadVipCards();
}
